...
#  include "include/zlib.h"  /* needed for MCCP */
# endif
#else
...
void send_gmcp(const char *, const char *);
void send_gmcp_to_server(const char *, const char *);
...
/* Table for our GMCP data*/
lookup_table_node *g_pGMCPdata_head;
...
void *get_function( char *name )
{
	...
	} functions[ ] =
	{
		...
		{ "mxp_stag", mxp_stag },
		{ "send_gmcp", send_gmcp },
		{ "send_gmcp_to_server", send_gmcp_to_server },
		/* Utility */
...
void module_process_server_gmcp_ex(const char *szModule, struct json_object *jData)
{
	MODULE *module;

	DEBUG("module_process_server_gmcp_module_ex");

	mod_section = "Processing server gmcp_ex";

	for (module = modules; module; module = module->next)
	{
		current_mod = module;
		if (module->process_server_gmcp_ex)
			(module->process_server_gmcp_ex)(szModule, jData);
	}
	mod_section = NULL;
}
...
void json_object_merge(json_object *jObject1, json_object *jObject2)
{

	json_object_object_foreach(jObject2, key, val)
	{
		json_object_get(val);
		json_object_object_add(jObject1, key, val);
	}
}


void parse_gmcp(const char *const szData)
{
	char *szModule = NULL;
	const char *szSubstr, *szSubstr2;
	int iSubstrLen, i;
	json_object *jPrevData, *jNewData;


	// Find the Module name, it'll start from the beginning until the first space.
	szSubstr = szData;
	szSubstr2 = strchr(szData, ' ');
	if (!szSubstr2) return;
	iSubstrLen = szSubstr2 - szSubstr;

	// Store the Module name
	szModule = (char *)malloc(sizeof(char) * (iSubstrLen + 1));
	strncpy(szModule, szSubstr, iSubstrLen);
	szModule[iSubstrLen] = '\0';

	// Find the Json string, it'll be after the space and until the end of the string.
	szSubstr = strchr(szData, ' ') + 1;
	iSubstrLen = strlen(szSubstr);

	// Get the existing module json data
	jPrevData = (json_object*)get_pdata(g_pGMCPdata_head, szModule);

	// If it doesn't exist, create it
	if (jPrevData == NULL)
		jPrevData = json_object_new_object();

	// Create the new json object.
	jNewData = json_tokener_parse(szSubstr);

	if (json_object_is_type(jNewData, json_type_object))
	{
		// Merge the new module into the current module.
		json_object_merge(jPrevData, jNewData);
		json_object_put(jNewData);
	}
	else
	{
		if (jPrevData != NULL)
			json_object_put(jPrevData);
		jPrevData = jNewData;
	}

	add_data(&g_pGMCPdata_head, szModule, (void**)jPrevData);

	module_process_server_gmcp_ex(szModule, jPrevData);
...
{
	const char iac_sb[] = { IAC, SB, 201, '\0' };
	const char iac_se[] = { IAC, SE, '\0' };

	clientf((char *)iac_sb);
	clientf((char *)szModule);
	clientf(" ");
	clientf((char *)szData);
	clientf((char *)iac_se);
}
...
void send_gmcp_to_server(const char *szModule, const char *szData)
{
	...
	send_to_server((char *)iac_sb);
	send_to_server((char *)szModule);
	send_to_server(" ");
	send_to_server((char *)szData);
	send_to_server((char *)iac_se);
}
	...
	for ( i = 0; i < bytes; i++ )
	{
		...
		if ( in_iac )
		{
					...
					strcmp( gmcp_login_as, "none" ) )
			{
				...
				sprintf(buf, "%s" "Core.Supports.Set [ \"Core 1\", \"Char 1\", \"Room 1\", \"IRE.Composer 1\", \"IRE.Rift 1\", \"Comm.Channel 1\", \"Char.Items 1\", \"Char.Skills 1\" ]" "%s",
						sb_gmcp, iac_se);
				send_to_server( buf );
				continue;
